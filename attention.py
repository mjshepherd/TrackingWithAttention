...
class ZoomableAttentionWindow(object):
    ...
    def filterbank_matrices(self, center_y, center_x, delta, sigma):
        ...
        # e.g.  [1.5, -0.5, 0.5, 1.5]
        rng = T.arange(N, dtype=floatX) - N / 2. + 0.5
        ...
        FX = tensor.exp(-(a - muX.dimshuffle([0, 1, 'x']))
                        ** 2 / 2. / sigma.dimshuffle([0, 'x', 'x'])**2)
        FY = tensor.exp(-(b - muY.dimshuffle([0, 1, 'x']))
                        ** 2 / 2. / sigma.dimshuffle([0, 'x', 'x'])**2)
    ...
    def read(self, images, center_y, center_x, delta, sigma):
        ...
        I = images.reshape(
            (batch_size * channels, self.img_height, self.img_width))
    ...
    def nn2att(self, l):
        ...
        delta = (max(self.img_width, self.img_height) - 1) / \
            (self.N - 1) * delta
...
if __name__ == "__main__":
    ...
    I, center_y, center_x, delta, sigma = \
        vectorize(I, np.array(center_y), np.array(
            center_x), np.array(delta), np.array(sigma))
